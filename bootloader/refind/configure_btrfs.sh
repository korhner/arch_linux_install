...
menuentry "Arch Linux" {
    ...
    options  "root=UUID=$system_uuid rw rootflags=subvol=root cryptdevice=PARTLABEL=cryptsystem:system cryptkey=PARTLABEL=decrypt:10240:256 quiet initrd=\\$MICROCODE.img initrd=\initramfs-linux.img"

    submenuentry "Boot using fallback initramfs" {
        options  "root=UUID=$system_uuid rw rootflags=subvol=root cryptdevice=PARTLABEL=cryptsystem:system cryptkey=PARTLABEL=decrypt:10240:256 quiet initrd=\\$MICROCODE.img initrd=\initramfs-linux-fallback.img"
    }
